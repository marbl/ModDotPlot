...
def create_map(kmer_list, resolution, sparsity):
    ...
    kmer_coordinates = create_coordinates(kmers, resolution)
...
def run_dash(kmer_list, resolution, sparsity, k, identity, port_number, jacc):
    # Run Dash app
    app = dash.Dash(__name__, prevent_initial_callbacks="initial_duplicate")
    current_color = COLOR_SCHEMES["Spectral"]

    # Create initial coordinates & figure
    init = create_map(kmer_list, resolution, sparsity)
    main_level = kmer_coordinates(init, False, resolution, k)
    increments = round(len(kmer_list) / resolution)
        ...
        try:
            if value:
                ...
                if (
                    ...
                    new_jaccard = []
                    x_mers = kmer_list[x_begin:x_end]
                    y_mers = kmer_list[y_end:y_begin]
                    new_map_x = create_map(x_mers, resolution, sparsity)
                    new_map_y = create_map(y_mers, resolution, sparsity)
