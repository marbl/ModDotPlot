from plotnine import ggsave, ggplot, aes, geom_histogram, geom_polygon, scale_color_discrete, scale_fill_gradientn, element_blank, theme, xlab, ggtitle, scale_fill_manual, scale_color_cmap, coord_cartesian, ylab, scale_x_continuous
import pandas as pd
import numpy as np
import math
...
def paired_bed_file(window_partitions: dict, input_name: str, id_threshold: int,
                     density: int, output: str, seq_length: int, no_plot: bool) -> None:
    """
...
def get_colors(sdf, ncolors, is_freq):
    ...
    breaks = np.unique(np.quantile(sdf['perID_by_events'], np.arange(0, 1.01, 1/ncolors)))
    labels = np.arange(len(breaks)-1)
    # corner case of only one %id value
    if len(breaks) == 1:
        return pd.factorize([1]*len(sdf['perID_by_events']))[0]
    return pd.cut(sdf['perID_by_events'], bins=breaks, labels=labels, include_lowest=True)
...
def read_bedpe(all_files):
    ...
    if '#query_name' in df.columns:
        df.rename(columns={'#query_name': 'q', 'query_start': 'q_st', 'query_end': 'q_en',
                           'reference_name': 'r', 'reference_start': 'r_st', 'reference_end': 'r_en'}, inplace=True)
...
def make_tri(df_d, title_name, seq_length):
    ...
    p_tri = (
        ggplot(df_d) + \
            ...
            #TODO: Replace this with built in color palettes.
            scale_fill_gradientn(colors=['#5E4FA2', '#3388BD', '#66C2A4', '#ABDEA4', '#E6F698', '#FFFFBF', '#FEE08B', '#FDAE61', '#F46D43', '#D53E4F', '#9F0142'], guide=False) + \
            theme(
...
def make_hist(sdf):
    ...
    p = (
        ...
        scale_fill_manual(['#5E4FA2', '#3388BD', '#66C2A4', '#ABDEA4', '#E6F698', '#FFFFBF', '#FEE08B', '#FDAE61', '#F46D43', '#D53E4F', '#9F0142']) +
